...
build_bedpe_with_metadata <- function(merged_dt) {
  ...
  merged_dt[, which_mate := unlist(strsplit(ID, ":"))[3], by = "uid"]
  temp_bedpe <- NULL
  removed_bnd <- NULL
...
merge <- function(lof_pth) {
  ...
  somatic_id <- unlist(strsplit(unlist(strsplit(path_somatic, "/"))[9], "[.]"))[1]
  germline_id <-  unlist(strsplit(unlist(strsplit(path_germline, "/"))[9], "[.]"))[1]
  
  if(somatic_id == germline_id) {
    output <- paste0("/xchip/beroukhimlab/Alex/CCLE_PCAWG/Figures_code/figure_data/data/bedpe_combined/", somatic_id, "_combined_germ_soma_test.bedpe")
