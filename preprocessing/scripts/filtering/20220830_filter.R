...
lof <- list.files('/xchip/beroukhimlab/siyun/germline_classifier/outputs/bedpe_combination') #folder containing the bedpe files
lof_pth <- paste0('/xchip/beroukhimlab/siyun/germline_classifier/outputs/bedpe_combination/', lof) 
...
for(i in 1:length(lof_pth)) {
  ...
  write.table(tmp_span, paste0('/xchip/beroukhimlab/siyun/germline_classifier/outputs/filtering/', lof[i],'.filtered'),
