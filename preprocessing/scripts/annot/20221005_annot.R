...
hg19_genes <- readRDS('/xchip/beroukhimlab/wolu/testing_svaba/extdata/hg19_geneRanges.rds')
#hg19_exons <- readRDS('/xchip/beroukhimlab/Alex/InProgress/inst/extdata/hg19_exonRanges.rds')
hg19_exons <- readRDS('/xchip/beroukhimlab/wolu/testing_svaba/extdata/hg19_exonRanges.rds')
...
chroms <-  as.character(c(1:24))
bedpe$chrom1 <- as.character(bedpe$chrom1)
bedpe$chrom2 <- as.character(bedpe$chrom2)
store_indices = which(!bedpe$chrom1 %in% chroms| !bedpe$chrom2 %in% chroms) #done to address the observation that some SVs were mapping to hpv
bedpe_clean <- bedpe[!store_indices,]
bedpe_clean$chrom1 <- as.numeric(bedpe_clean$chrom1)
bedpe_clean$chrom2 <- as.numeric(bedpe_clean$chrom2)
bedpe_clean$start1 <- as.numeric(bedpe_clean$start1)
bedpe_clean$start2 <- as.numeric(bedpe_clean$start2)

cat("Reading file...")
bedpe_formated <- rbindlist(lapply(1:nrow(bedpe_clean), check_reformat, bedpe_clean))
cat("done. \n")
...
sample_output <- paste0('/xchip/beroukhimlab/wolu/testing_svaba/outputs/final_annot/annotated_only/', unlist(strsplit(filepath,'fuzzymatch_only/'))[2], '.annot.bedpe')
